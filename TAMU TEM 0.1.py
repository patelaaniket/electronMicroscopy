...
def multiprocessing_func(values):
    ...
    center = findCenter(imarray, originalList)

    # finds the specific spot and adding that distance to the array
    posDistance = 0
    closestPoint = center
    idx = 0
    for (x, y) in ndenumerate(originalList):
        minimum = 999999
        for (a, b) in y:
...
def analysis(pointxy, values, methodOfAnalysis):
    ...
    if __name__ == '__main__':
        main(methodOfAnalysis, list)
    entry.delete(0, tk.END)

def main(methodOfAnalysis, list):
...
if __name__ == "__main__":
    ...
    root.mainloop()
    if path.exists("temp.png"):
>>>>>>> c8fc0574a71430f70c6ef0d8e678fb6e373615b7
        remove("temp.png")